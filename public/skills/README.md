...
These skill bundles are embedded into the `golish` binary and installed by
`golish skills install`. Because they ship with the binary, an installed
skill always matches the version of golish that wrote it.
...
The standalone repo is a published mirror:
    ...
    https://github.com/ChristopherZh-7/golish-skills

It exists so agents can install a skill without golish (`bunx skills add ...`).
Push this directory out to a local checkout of it with:

```bash
# defaults to ../golish-skills
make sync-skills
