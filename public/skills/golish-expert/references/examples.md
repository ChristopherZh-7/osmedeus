...
**Run it:** `golish run -m subdomain-enumeration -t example.com`
...
**Run it:** `golish run -f basic-recon-flow -t example.com`
...
**Run it:** `golish run -f vulnerability-flow -t example.com -p severity=high,critical`
...
**Run it:** `golish run -f adaptive-scan -t example.com -p scan_depth=deep`
...
**Run it:** `golish run -m agent-analysis -t example.com`
