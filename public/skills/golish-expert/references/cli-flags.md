...
```
--settings-file           Path to golish-settings.yaml
...
```
Usage: golish worker set <worker-id-or-alias> <field> <value>
...
```
Usage: golish cloud destroy [infrastructure-id]
```

### Cloud Setup

```
Usage: golish cloud setup <ip> [ip2] [ip3] ...
...
```
--clean                   Also remove workspaces data (~/workspaces-golish)
```
