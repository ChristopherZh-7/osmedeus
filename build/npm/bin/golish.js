#!/usr/bin/env node
// Unified entry point for the Golish CLI distributed via npm.
//
// The platform-specific binary is shipped *gzipped* inside an optional
// dependency package (one of @christopherzh-7/golish-<tag>). On first run we
...
// published to npm is always @christopherzh-7/golish with a version suffix.
const PLATFORM_PACKAGE_BY_TAG = {
  "linux-x64": "@christopherzh-7/golish-linux-x64",
  "linux-arm64": "@christopherzh-7/golish-linux-arm64",
  "darwin-x64": "@christopherzh-7/golish-darwin-x64",
  "darwin-arm64": "@christopherzh-7/golish-darwin-arm64",
};
...
function reinstallHint() {
  return detectPackageManager() === "bun"
    ? "bun install -g @christopherzh-7/golish@latest"
    : "npm install -g @christopherzh-7/golish@latest";
}
...
if (!platformTag) {
  throw new Error(
    `Unsupported platform: ${platform} (${arch}). ` +
      `Golish npm builds cover linux/darwin on x64/arm64. ` +
      `See https://github.com/ChristopherZh-7/golish-pentest-platform/tree/main/docs for other install options.`,
  );
}
...
if (!gzPath || !existsSync(gzPath)) {
  throw new Error(
    `Missing platform package ${platformPackage} (the golish binary for ` +
      `${platformTag} was not installed). This usually means npm skipped ` +
      `optional dependencies. Reinstall Golish:\n    ${reinstallHint()}`,
  );
}
...
const golishHome =
  process.env.GOLISH_NPM_HOME || path.join(os.homedir(), ".golish");
const binaryDir = path.join(golishHome, "npm-bin", pkgVersion, platformTag);
const binaryPath = path.join(binaryDir, "golish");
...
async function ensureBinary() {
  ...
  const tmpPath = path.join(
    binaryDir,
    `golish.${process.pid}.${Date.now()}.tmp`,
  );
  ...
  try {
    ...
    throw new Error(
      `Failed to unpack the golish binary for ${platformTag}: ${err.message}\n` +
        `Try reinstalling:\n    ${reinstallHint()}`,
    );
  }
}
