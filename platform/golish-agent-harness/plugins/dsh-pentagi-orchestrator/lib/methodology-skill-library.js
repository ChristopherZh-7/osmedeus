...
export const METHODOLOGY_SKILL_PROVIDER = "methodology-index";
export const METHODOLOGY_SKILL_NOTICE = "> **Golish DSH adaptation:** Load `golish-pentest` first and keep every action inside its authorized asset allowlist. Treat upstream-specific tool names as methodology references unless the tool is actually available. Obtain explicit operator approval before destructive, persistence, denial-of-service, credential-spraying, or data-exfiltration actions.";
...
function abortIfRequested(signal) {
  ...
  if (signal?.aborted) throw signal.reason ?? new Error("Methodology Skill indexing was aborted");
}
...
export function isMethodologySkillName(value) {
  const name = String(value || "").trim();
  return name.length > 0 && name.length <= 512 && !/[\u0000-\u001F\u007F]/.test(name);
}

export function parseMethodologySkill(text, location = "SKILL.md") {
  ...
  if (!name || !isMethodologySkillName(name)) throw new Error(`${location} has an invalid Skill name`);
...
export function defaultMethodologySkillRoot(env = process.env) {
  if (String(env.GOLISH_METHODOLOGY_SKILLS_DIR || "").trim()) {
    return resolve(String(env.GOLISH_METHODOLOGY_SKILLS_DIR).trim());
  }
  const dshHome = resolve(String(
    env.DSH_HOME || join(homedir(), "golish-base", "agent-harness", "dsh-home"),
  ));
  return join(dshHome, "golish", "methodology-skills");
}
...
async function parseIndexedFile(root, path, signal) {
  ...
  const parsed = parseMethodologySkill(
...
export async function buildMethodologySkillIndex(rootInput, { signal, concurrency = 32 } = {}) {
...
export function searchMethodologySkillIndex(index, options = {}) {
...
export class MethodologySkillLibrary {
  ...
  root() {
    return this.configuredRoot ? resolve(this.configuredRoot) : defaultMethodologySkillRoot(this.env);
  }
  ...
  async ensureIndex(signal) {
    ...
    if (!this.indexPromise) {
      this.indexPromise = buildMethodologySkillIndex(root, { signal })
        .then((index) => {
          this.index = index;
          return index;
        })
  ...
  async status(signal) {
    ...
    return {
      action: "status",
      provider: METHODOLOGY_SKILL_PROVIDER,
  ...
  async search(options = {}, signal) {
    const index = await this.ensureIndex(signal);
    const result = searchMethodologySkillIndex(index, options);
    return {
      action: "search",
      provider: METHODOLOGY_SKILL_PROVIDER,
      available: index.available,
  ...
  async load(name, signal) {
    ...
    if (!relativeToRoot || relativeToRoot.startsWith(`..${sep}`) || relativeToRoot === "..") {
      throw new Error("Methodology Skill path escaped the configured library root");
    }
    const info = await stat(path);
    if (!info.isFile() || info.size > MAX_SKILL_BYTES) {
      throw new Error(`Methodology Skill ${name} is not a loadable file`);
    }
    const parsed = parseMethodologySkill(
      await readFile(path, { encoding: "utf8", signal }),
      path,
    ...
    if (parsed.name !== name) {
      this.index = undefined;
      throw new Error(`Methodology Skill ${name} changed after indexing; refresh and retry`);
    }
    return {
      action: "load",
      provider: METHODOLOGY_SKILL_PROVIDER,
      ...
      content: `${METHODOLOGY_SKILL_NOTICE}\n\n${parsed.content}`,
      prerequisites: parsed.prerequisites,
      chains_with: parsed.chains_with,
    };
  }
