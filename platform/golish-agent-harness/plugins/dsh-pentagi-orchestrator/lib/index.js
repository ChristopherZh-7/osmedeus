...
import { pentestBridgeContext, postPentestBridge } from "@golish/dsh-plugin";
import { z } from "zod";

import { MethodologySkillLibrary, isMethodologySkillName } from "./methodology-skill-library.js";
import { ROLE_IDS, ROLE_REGISTRY, roleDefinition } from "./roles.js";

const ORCHESTRATION_ROUTE = "/golish/api/agent-pentest/bridge/orchestration";
...
const ROOT_LEGACY_TASK_TOOLS = new Set([
  "golish_start_pentest_task",
  "golish_get_pentest_task",
  "golish_resume_pentest_task",
  "golish_steer_pentest_task",
  "golish_cancel_pentest_task",
]);
...
export function pentestPrimaryPrompt(mode) {
  const identity = [
    "You are Primary Agent, the single operator-facing agent for this authorized Golish pentest session.",
    ...
    "Load the golish-pentest Skill before target work, read frozen scope with pentagi_context, stay inside the allowlist, prefer low-impact verification, preserve evidence, and answer in this same conversation.",
    "Use pentagi_skill search to find a Methodology methodology for a concrete hypothesis, then load only the selected Skill body. Never list or load the whole Methodology corpus into context.",
    "Use the native todo tool for a useful visible plan. Do not call any golish_*_pentest_task tool; that legacy background state machine is not the interactive product model.",
  ];
...
export function pentestRootToolDenial(mode, toolName) {
  if (mode === "analysis") {
    ...
    return `This Golish session uses read-only analysis mode. ${toolName} is disabled. Use pentagi_context or skill to inspect existing reconnaissance, then answer without executing tools, changing targets, writing files, starting tasks, or submitting results.`;
  }
  if (ROOT_NATIVE_DELEGATION_TOOLS.has(toolName)) {
    return `The Primary agent cannot call ${toolName} in this Golish session. ${mode === "orchestrated" ? "Use pentagi_delegate for a named security specialist." : "Solo mode does not permit subagents."}`;
  }
...
const methodologySkillLibrary = new MethodologySkillLibrary();
...
function taskNotice(agent, text, summary, wake = false) {
  ...
  const message = createUserMessage({
    ...
    source: {
      kind: "plugin",
      plugin: "@golish/dsh-pentagi-orchestrator",
      form: "notice",
...
function rolePrompt(roleID, input, bridge, retryGuidance = "", allowedToolNames) {
  ...
  const toolInstructions = tools.length
    ? [
      ...
      tools.includes("pentagi_skill")
        ? "First call pentagi_skill with name golish-pentest. For a concrete security hypothesis, search the indexed Methodology library and load only the most relevant Skill body."
  ...
  return [
    ...
    `Canonical Golish context: ${bridge.contextPath}`,
...
async function runPersistentPrimary(ctx, options) {
  ...
  try {
    ...
    await ctx.subagents.followup(
      ...
      {
        source: { kind: "plugin", plugin: "@golish/dsh-pentagi-orchestrator", form: "relay" },
        signal,
      },
    );
  ...
  try {
    ...
    if (detail.task.plan_revision === 0) {
      const generator = await runRole(ctx, {
        ...
        input: { objective, existing_recon: "Read the canonical Golish context before planning." },
      });
      ...
      taskNotice(
        rootAgent,
        `PentAGI task ${taskUUID} generated a ${items.length}-item plan. Do not start another task. Use golish_get_pentest_task if the operator asks for status.`,
    ...
    for (let cycle = 0; cycle < MAX_TASK_CYCLES; cycle += 1) {
      ...
      taskNotice(
        rootAgent,
        `PentAGI task ${taskUUID} started subtask ${currentSubtask.uuid}: ${currentSubtask.title}. Operator messages about this task should be routed with golish_steer_pentest_task.`,
      ...
      if (primaryStatus === "ask") {
        ...
        taskNotice(
          rootAgent,
          `PentAGI task ${taskUUID} is waiting for operator input: ${question}\nCall golish_get_pentest_task once, explain the blocker to the operator, and do not start another task.`,
          `任务等待输入：${question}`,
          true,
        );
        return;
    ...
    taskNotice(
      rootAgent,
      `PentAGI task ${taskUUID} completed. Call golish_get_pentest_task exactly once, then give the operator a concise final report covering verified findings, tested coverage, and remaining risk. Do not start or resume another task.`,
      `任务 ${taskUUID} 已完成`,
      true,
    );
  } catch (error) {
    ...
    taskNotice(
      rootAgent,
      `PentAGI task ${taskUUID} ${cancelled ? "was cancelled" : "failed"}: ${renderError(error)}. Call golish_get_pentest_task once and explain this terminal state to the operator. Do not start another task automatically.`,
...
function registerPrimaryPrompt(ctx) {
  ctx.systemPrompt.section({
    name: "golish:interactive-primary",
    order: 12,
    text: ({ agent }) => {
      if (!agent) return "";
      try {
...
function registerMemoryTools(ctx) {
  ctx.tools.register(defineTool({
    ...
    description: "Search concise persisted memory using Golish-scoped hybrid keyword and semantic retrieval when configured. The default scope is the current Pentest Session; task, workspace, and org scopes can be selected explicitly. Inspect retrieval_mode, scores, and degraded_reason. Memory is context, never proof or authorization.",
    parameters: {
...
function registerContextTool(ctx) {
  ctx.tools.register(defineTool({
    ...
    description: "Read canonical immutable Golish scope and reconnaissance for this Golish root session or a managed PentAGI role. Use this instead of guessing workspace paths or reading reconnaissance files directly.",
    parameters: {
      section: {
        type: "string",
        required: true,
...
function registerSkillTool(ctx) {
  ctx.tools.register(defineTool({
    name: "pentagi_skill",
    description: "Search and lazily load the indexed Methodology methodology library without placing thousands of Skill summaries in the DSH catalog. Also loads ordinary installed DSH Skills by exact name. Always load golish-pentest first, search for a concrete hypothesis, and load only the selected body.",
    parameters: {
      action: {
        type: "string",
      ...
      name: {
        type: "string",
        description: "Exact installed or indexed Skill name for load/chain, such as golish-pentest, attack-ssrf, or wstg-inpv-05.",
      },
      query: { type: "string", description: "Keyword, Skill name fragment, OWASP/CIS identifier, or description text." },
      category: { type: "string", description: "Exact Methodology category filter." },
      cwe: { type: "string", description: "Exact CWE identifier filter, such as CWE-89." },
      tag: { type: "string", description: "Exact Methodology tag filter." },
    ...
    async execute(args, exec) {
      ...
      if (["load", "chain"].includes(action)) {
        if (!isMethodologySkillName(args.name)) {
          throw new Error(`A valid Skill name is required for ${action}`);
        }
      }
      if (action === "status") return methodologySkillLibrary.status(exec.signal);
      if (action === "refresh") {
        await methodologySkillLibrary.refresh(exec.signal);
        return methodologySkillLibrary.status(exec.signal);
      }
      if (action === "search" || action === "list") {
        const result = await methodologySkillLibrary.search({
        ...
        if (!result.available) {
          result.setup_hint = "Configure GOLISH_METHODOLOGY_SKILLS_DIR or install the corpus below $DSH_HOME/golish/methodology-skills.";
        }
        return result;
      }
      if (action === "chain") {
        const chain = await methodologySkillLibrary.chains(args.name, exec.signal);
        if (!chain) throw new Error(`Indexed Methodology Skill ${args.name} is not available`);
        return chain;
      }
      ...
      const indexed = await methodologySkillLibrary.load(args.name, exec.signal);
      if (indexed) return indexed;
      const status = await methodologySkillLibrary.status(exec.signal);
      if (!status.available) {
        throw new Error(`Skill ${args.name} is not installed and the indexed Methodology library is not configured`);
      }
      throw new Error(`Skill ${args.name} is not available in installed DSH Skills or the indexed Methodology library`);
    },
  }));
}
...
function registerTaskTools(ctx) {
  ctx.tools.register(defineTool({
    name: "golish_start_pentest_task",
    description: "Start a durable PentAGI-style multi-role penetration-test task against only this Golish session's frozen assets. Returns immediately while the task runs in the background.",
    parameters: {
  ...
  ctx.tools.register(defineTool({
    name: "golish_steer_pentest_task",
  ...
  ctx.tools.register(defineTool({
    name: "golish_resume_pentest_task",
  ...
  ctx.tools.register(defineTool({
    name: "golish_cancel_pentest_task",
...
export function apply(ctx) {
  ...
  ctx.logger.info(`golish PentAGI orchestrator loaded with ${ROLE_IDS.length} logical roles`);
}
