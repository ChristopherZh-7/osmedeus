...
const CONTEXT_ROUTE = "/golish/context";
const CONTEXT_STATUS_ROUTE = "/golish/context/status";
const QUESTION_ROUTE = "/golish/questions";
const QUESTION_RESPONSE_ROUTE = "/golish/questions/respond";
const MAX_CONTEXT_BYTES = 16 * 1024 * 1024;
const SESSION_ID_PATTERN = /^session-golish-[a-f0-9]{32}$/;
const bridgeByRootSession = new Map();
const golishAPI = new URL(process.env.GOLISH_API_URL || "http://127.0.0.1:8002");
...
function registerQuestionBridge(ctx) {
  ...
  const watch = async () => {
    try {
      ...
      if (!abort.signal.aborted) console.error("[golish] question bridge stopped:", error);
    }
  };
  void watch();
  ctx.effect(() => () => {
    ...
    () => ctx.webServer.register({
      ...
      handler(req, res) {
        ...
        if (!SESSION_ID_PATTERN.test(sessionId)) {
          sendJSON(res, 400, { ok: false, error: "invalid Golish DSH session id" });
          return;
        }
        sendJSON(res, 200, {
          ok: true,
  ...
  ctx.effect(
    ...
    "golish: question response bridge",
  );
}
...
export function findBridgeForAgent(ctx, agent) {
  ...
  throw new Error(
    "No active Golish authorization scope was found for this agent. Open the root Agent Pentest Session and click Sync Recon.",
  );
}
...
export async function postPentestBridge(ctx, agent, signal, route, payload) {
	...
	const endpoint = new URL(route, golishAPI);
  const response = await fetch(endpoint, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
      "X-Golish-Pentest-Token": bridge.token,
    },
    body: JSON.stringify({
      ...payload,
  ...
  if (!response.ok) {
    throw new Error(value?.message || `Golish result bridge returned HTTP ${response.status}`);
  }
	return value;
}
...
function registerResultTools(ctx) {
  ctx.tools.register(
    defineTool({
      name: "golish_record_test",
      description:
        "Record the latest coverage status for a concrete test performed against an asset in the frozen Golish scope. Call after an actual test attempt (including passed, blocked, or skipped), not while merely planning.",
      parameters: {
        asset_id: {
          type: "number",
          required: true,
      ...
      output: {
        ...
        render: (_args, value) => [
          {
            type: "text",
            text: `Golish coverage recorded: ${value?.data?.status || "saved"}`,
          },
        ],
      },
      execute: (args, exec) =>
        postBridge(ctx, exec, "/golish/api/agent-pentest/bridge/coverage", args),
    }),
  );

  ctx.tools.register(
    defineTool({
      name: "golish_submit_finding",
      description:
        "Submit a reproducible vulnerability candidate from an actual authorized test to Golish Vulnerabilities. It is stored as pending review; never use this for an unverified scanner lead or hypothesis.",
      parameters: {
        asset_id: {
          type: "number",
          required: true,
        ...
        confidence: {
          type: "string",
          description: "Agent's confidence rationale; Golish still stores the candidate as Manual Review Required.",
        },
        ...
        render: (_args, value) => [
          {
            ...
            text: value?.deduplicated
              ? `Golish finding already exists as #${value?.data?.id}`
              : `Golish pending finding created as #${value?.data?.id}`,
          },
        ],
      },
      execute: (args, exec) =>
        postBridge(ctx, exec, "/golish/api/agent-pentest/bridge/findings", args),
    }),
  );
}
...
function contextSummary(context, contextPath, rootSessionId = context?.session?.dsh_session_id) {
  ...
  return [
    ...
    "",
    "Load the `golish-pentest` Skill before acting on this context.",
...
async function writeAgentContext(scopesRoot, sessionId, bridge) {
	...
	await writeFile(
		metadataPath,
		`${JSON.stringify({
			schema_version: "golish.pentest-scope/v1",
			session_id: sessionId,
...
/**
 * Materialize Golish reconnaissance under DSH_HOME. Harness injects
...
export function apply(ctx) {
  ...
  const scopesRoot = join(dshHome, "golish", "scopes");
        ...
        async handler(req, res) {
          ...
          try {
            ...
            if (
              ...
              payload?.context?.schema_version !== "golish.pentest-context/v1"
            ) {
            ...
            if (bridgeToken.length < 32 || bridgeToken.length > 512) {
              sendJSON(res, 400, { ok: false, error: "invalid Golish bridge token" });
              return;
            }
