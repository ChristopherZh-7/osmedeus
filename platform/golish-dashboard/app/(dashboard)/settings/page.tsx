...
export default function SettingsPage() {
  ...
  React.useEffect(() => {
    ...
    const savedEndpoint = localStorage.getItem("golish_api_endpoint") || "";
    const savedToken = localStorage.getItem("golish_token") || "";
    const savedPrefixKey = localStorage.getItem("golish_workspace_prefix_key") || "";
    const savedPreset = localStorage.getItem("golish_theme_preset");
    setEndpoint(savedEndpoint);
    setToken(savedToken);
    setPrefixKey(savedPrefixKey);
    if (savedPreset) setPreset(savedPreset);
    setSidebarCollapsedByDefault(localStorage.getItem("golish_sidebar_collapsed_by_default") === "true");
    ...
    const normalized =
      trimmed === API_PREFIX
        ...
        : trimmed.endsWith(API_PREFIX)
          ...
          : trimmed;
    localStorage.setItem("golish_api_endpoint", normalized.startsWith("/") ? window.location.origin : normalized);
    localStorage.setItem("golish_token", token);
    localStorage.setItem("golish_workspace_prefix_key", prefixKey.trim());
    toast.success("控制台连接已保存");
  };

  const savePreset = (value: string) => {
    setPreset(value);
    localStorage.setItem("golish_theme_preset", value);
    localStorage.removeItem("golish_theme_light_primary");
    localStorage.removeItem("golish_theme_light_secondary");
    localStorage.removeItem("golish_theme_dark_primary");
    localStorage.removeItem("golish_theme_dark_secondary");
    window.dispatchEvent(new Event("golish-theme-colors-updated"));
    toast.success("主题已应用");
  };

  const saveSidebarCollapsed = (checked: boolean) => {
    setSidebarCollapsedByDefault(checked);
    localStorage.setItem("golish_sidebar_collapsed_by_default", checked ? "true" : "false");
    window.dispatchEvent(new CustomEvent("golish-sidebar-collapsed-by-default-changed", { detail: checked }));
    ...
    <div className="space-y-4">
      ...
      <TabsContent value="skills" className="space-y-4">
        ...
        <div className="rounded-control border bg-muted/25 p-3 text-sm text-muted-foreground">
          安装全部编码助手 Skills：<code className="ml-1 rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-foreground">golish skills install --agent codex --all</code>
        </div>
      </TabsContent>

      <TabsContent value="system" className="space-y-4">
        <Card className="overflow-hidden">
          <SectionCardHeader icon={PlugIcon} title="控制台连接" description="仅保存在当前浏览器，用于连接 Golish API 与打开原始产物" />
          <CardContent className="space-y-4">
          ...
          <CardContent>
            <Collapsible open={yamlOpen} onOpenChange={setYamlOpen}>
              <CollapsibleTrigger className="flex w-full items-center justify-between rounded-control border bg-muted/25 p-4 text-left">
                <div><div className="font-medium">查看 golish-settings.yaml</div><div className="mt-1 text-sm text-muted-foreground">敏感值会在服务端递归替换为 [REDACTED]</div></div>
