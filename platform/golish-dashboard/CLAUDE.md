...
Golish Dashboard - A UI dashboard for the Golish Workflow Engine built with Next.js App Router, Tailwind CSS v4, and Shadcn UI (new-york style).
...
### Authentication
Mock auth via `providers/auth-provider.tsx` using localStorage (`golish_session` key). Default credentials: any username with 4+ character password. The `AuthProvider` handles redirect logic between public/protected routes.
