...
```bash
curl http://localhost:8002/golish/api/vulnerabilities \
  -H "Authorization: Bearer $TOKEN"
```

**List vulnerabilities with pagination:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?offset=0&limit=100" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by workspace:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by severity:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?severity=critical" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by confidence:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?confidence=Certain" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by asset value (partial match):**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?asset_value=api.example" \
  -H "Authorization: Bearer $TOKEN"
```

**Combine filters:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities?workspace=example.com&severity=high&offset=0&limit=50" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/vulnerabilities/summary \
  -H "Authorization: Bearer $TOKEN"
```

**Get summary for a specific workspace:**
```bash
curl "http://localhost:8002/golish/api/vulnerabilities/summary?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/vulnerabilities/1 \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST http://localhost:8002/golish/api/vulnerabilities \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
curl -X DELETE http://localhost:8002/golish/api/vulnerabilities/15 \
  -H "Authorization: Bearer $TOKEN"
```
