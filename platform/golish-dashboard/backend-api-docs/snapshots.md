...
```bash
curl http://localhost:8002/golish/api/snapshots \
  ...
  "data": [
    {
      ...
      "path": "/home/user/golish-base/snapshot/example.com_1704067200.zip",
      "size": 15728640,
      "created_at": "2025-01-01T12:00:00Z"
    },
    {
      "name": "test.com_1704153600.zip",
      "path": "/home/user/golish-base/snapshot/test.com_1704153600.zip",
      "size": 8388608,
      "created_at": "2025-01-02T12:00:00Z"
    }
  ],
  "count": 2,
  "path": "/home/user/golish-base/snapshot"
}
```
...
```bash
curl -X POST http://localhost:8002/golish/api/snapshots/export \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
curl -X POST http://localhost:8002/golish/api/snapshots/import \
  -H "Authorization: Bearer $TOKEN" \
...
```json
{
  ...
  "local_path": "/home/user/workspaces-golish/example.com",
...
{
  "error": true,
  "message": "Failed to import snapshot: workspace already exists: /home/user/workspaces-golish/example.com (use --force to overwrite)"
}
```
...
```bash
curl -X DELETE http://localhost:8002/golish/api/snapshots/example.com_1704067200.zip \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/snapshot-download/example.com \
  -H "Authorization: Bearer $TOKEN" \
  --output snapshot.zip
```
