...
```bash
curl -X POST http://localhost:8002/golish/api/login \
...
As an alternative to JWT tokens, you can authenticate using a static API key via the `x-golish-api-key` header. This is useful for scripts, CI/CD pipelines, or integrations where managing JWT token refresh is impractical.

### Configuration

API key authentication is configured in `~/golish-base/golish-settings.yaml`:

```yaml
server:
  # Enable API key authentication (default: true)
  enabled_auth_api: true
  # API key for x-golish-api-key header authentication
...
Include the API key in requests using the `x-golish-api-key` header:

```bash
# Store API key in environment variable
export GOLISH_API_KEY="your-api-key-here"

# Use in API requests
curl http://localhost:8002/golish/api/workflows \
  -H "x-golish-api-key: $GOLISH_API_KEY"
```
