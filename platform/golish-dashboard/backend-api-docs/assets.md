...
**List all assets:**
```bash
curl http://localhost:8002/golish/api/assets \
  -H "Authorization: Bearer $TOKEN"
```

**List assets with pagination:**
```bash
curl "http://localhost:8002/golish/api/assets?offset=0&limit=100" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by workspace:**
```bash
curl "http://localhost:8002/golish/api/assets?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```

**Combine workspace filter with pagination:**
```bash
curl "http://localhost:8002/golish/api/assets?workspace=example.com&offset=50&limit=25" \
  -H "Authorization: Bearer $TOKEN"
```
...
| `remarks` | array | Custom labels/tags/remarks |
| `language` | string | Repository/source language |
| `size` | int | Repository/file size in bytes |
| `loc` | int | Lines of code (for repository assets) |
| `blob_content` | string | Large content (raw files, keys, etc.) |
...
**Get all asset stats:**
```bash
curl http://localhost:8002/golish/api/asset-stats \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by workspace:**
```bash
curl "http://localhost:8002/golish/api/asset-stats?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/assets/diffs \
  -H "Authorization: Bearer $TOKEN"
```

**List with pagination:**
```bash
curl "http://localhost:8002/golish/api/assets/diffs?offset=0&limit=50" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by workspace:**
```bash
curl "http://localhost:8002/golish/api/assets/diffs?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```

**Combine workspace filter with pagination:**
```bash
curl "http://localhost:8002/golish/api/assets/diffs?workspace=example.com&offset=0&limit=25" \
  -H "Authorization: Bearer $TOKEN"
```
