...
```bash
curl http://localhost:8002/golish/api/settings/yaml \
...
```bash
curl -X PUT http://localhost:8002/golish/api/settings/yaml \
  -H "Authorization: Bearer $TOKEN" \
...
```json
{
  ...
  "path": "/home/user/golish-base/golish-settings.yaml",
  "backup": "/home/user/golish-base/golish-settings.yaml.backup"
}
```
