...
```bash
curl http://localhost:8002/golish/api/workflows \
...
```bash
# Filter by tags
curl "http://localhost:8002/golish/api/workflows?tags=recon,subdomain" \
  -H "Authorization: Bearer $TOKEN"

# Filter by kind (module)
curl "http://localhost:8002/golish/api/workflows?kind=module" \
  -H "Authorization: Bearer $TOKEN"


# Search workflows
curl "http://localhost:8002/golish/api/workflows?search=enum" \
  -H "Authorization: Bearer $TOKEN"

# Load directly from filesystem (bypasses database)
curl "http://localhost:8002/golish/api/workflows?source=filesystem" \
  -H "Authorization: Bearer $TOKEN"

# Pagination
curl "http://localhost:8002/golish/api/workflows?offset=10&limit=20" \
  -H "Authorization: Bearer $TOKEN"
```
  ...
  "data": [
    {
      ...
      "file_path": "/home/user/golish-base/workflows/flows/subdomain-enum.yaml",
      "params": [
    ...
    },
    {
      ...
      "file_path": "/home/user/golish-base/workflows/modules/port-scan.yaml",
      "params": [
        {"name": "target", "required": true, "default": "", "generator": ""},
...
```bash
curl http://localhost:8002/golish/api/workflows/tags \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST http://localhost:8002/golish/api/workflows/refresh \
  -H "Authorization: Bearer $TOKEN"
```
...
**Force re-index all:**
```bash
curl -X POST "http://localhost:8002/golish/api/workflows/refresh?force=true" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl http://localhost:8002/golish/api/workflows/subdomain-enum \
  -H "Authorization: Bearer $TOKEN"
```

```bash
# Get workflow details as JSON
curl "http://localhost:8002/golish/api/workflows/subdomain-enum?json=true" \
  -H "Authorization: Bearer $TOKEN"
```
