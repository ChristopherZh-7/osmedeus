...
curl -X POST http://localhost:8002/golish/api/runs \
  ...
  -d '{
    ...
    "target": "example.com",
    "runner_type": "docker",
    "docker_image": "golish/golish:latest"
  }'
```
...
Use an uploaded target file (from `/golish/api/upload-file`) for running:
...
curl -X POST http://localhost:8002/golish/api/runs \
  ...
  -d '{
    "module": "port-scan",
    "target_file": "/home/user/golish-base/data/uploads/targets.txt",
    "concurrency": 5
  }'
```

This is similar to CLI's `-T` flag: `golish run -m port-scan -T targets.txt`
...
```bash
curl http://localhost:8002/golish/api/runs \
  -H "Authorization: Bearer $TOKEN"
```
  ...
  "data": [
    {
      ...
      "params": {"threads": "50"},
      "status": "running",
      "workspace_path": "/home/user/golish-base/workspaces/example.com",
...
```bash
curl http://localhost:8002/golish/api/runs/run-abc123 \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X DELETE http://localhost:8002/golish/api/runs/run-abc123 \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/runs/run-abc123/steps \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/runs/run-abc123/artifacts \
  -H "Authorization: Bearer $TOKEN"
```
