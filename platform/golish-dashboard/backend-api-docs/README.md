# Golish API Documentation

## Overview

The Golish API provides a RESTful interface for managing security automation workflows, runs, and distributed task execution.
...
2. **API Key**: Use a static API key via the `x-golish-api-key` header. Configure in `~/golish-base/golish-settings.yaml` under `server.auth_api_key`.
...
# Login and get token
export TOKEN=$(curl -s -X POST http://localhost:8002/golish/api/login \
  -H "Content-Type: application/json" \
  -d '{"username": "golish", "password": "admin"}' | jq -r '.token')
