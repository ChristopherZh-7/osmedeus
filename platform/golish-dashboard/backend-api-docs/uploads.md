...
```bash
curl -X POST http://localhost:8002/golish/api/upload-file \
...
```json
{
  ...
  "path": "/home/user/golish-base/data/uploads/1704326400000000000_targets.txt",
  "size": 1024,
  "lines": 50
}
```
...
```bash
curl -X POST http://localhost:8002/golish/api/workflow-upload \
  -H "Authorization: Bearer $TOKEN" \
...
```json
{
  ...
  "path": "/home/user/golish-base/workflows/modules/my-custom-workflow.yaml"
}
```
