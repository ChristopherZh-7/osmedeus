...
```bash
curl http://localhost:8002/golish/api/registry-info \
...
```json
{
  ...
  "registry_url": "https://raw.githubusercontent.com/ChristopherZh-7/golish-base/main/registry-metadata.json",
  "binaries": {
    "nuclei": {
...
```bash
curl "http://localhost:8002/golish/api/registry-info?registry_mode=nix-build" \
  -H "Authorization: Bearer $TOKEN"
```
...
**Endpoint:** `POST /golish/api/registry-install`
...
```bash
curl -X POST http://localhost:8002/golish/api/registry-install \
  -H "Authorization: Bearer $TOKEN" \
...
curl -X POST http://localhost:8002/golish/api/registry-install \
  ...
  -d '{
    "type": "workflow",
    "source": "https://github.com/ChristopherZh-7/golish-workflow.git"
  }'
```
