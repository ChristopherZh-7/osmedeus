...
```bash
curl http://localhost:8002/golish/api/workspaces \
  -H "Authorization: Bearer $TOKEN"
```

**List workspaces with pagination:**
```bash
curl "http://localhost:8002/golish/api/workspaces?offset=0&limit=50" \
  -H "Authorization: Bearer $TOKEN"
```

**List workspaces from filesystem/assets:**
```bash
curl "http://localhost:8002/golish/api/workspaces?filesystem=true" \
  -H "Authorization: Bearer $TOKEN"
```

**Combine pagination with filesystem mode:**
```bash
curl "http://localhost:8002/golish/api/workspaces?filesystem=true&offset=20&limit=10" \
  -H "Authorization: Bearer $TOKEN"
```
  ...
  "data": [
    {
      "id": 1,
      "name": "example.com",
      "local_path": "/home/user/golish-base/workspaces/example.com",
      "total_assets": 150,
    ...
    },
    {
      "id": 2,
      "name": "test.com",
      "local_path": "/home/user/golish-base/workspaces/test.com",
      "total_assets": 50,
      "total_subdomains": 35,
      "total_urls": 120,
      "total_vulns": 3,
...
**Get execution log:**
```bash
curl "http://localhost:8002/golish/api/workspaces/example.com/state-file?state_file=execution_log" \
  -H "Authorization: Bearer $TOKEN"
```

**Get completed file:**
```bash
curl "http://localhost:8002/golish/api/workspaces/example.com/state-file?state_file=completed_file" \
  -H "Authorization: Bearer $TOKEN"
```

**Get workflow file:**
```bash
curl "http://localhost:8002/golish/api/workspaces/example.com/state-file?state_file=workflow_file" \
  -H "Authorization: Bearer $TOKEN"
```
...
```json
{
  ...
  "file_path": "/home/user/golish-base/workspaces/example.com/log/execution.log",
