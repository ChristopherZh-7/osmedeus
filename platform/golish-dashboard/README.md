...
UI dashboard for the Golish Workflow Engine, built with Next.js App Router, Tailwind CSS v4, and Shadcn UI.
...
- Mock authentication (client-side session in `localStorage` under `golish_session`)
...
- Session stored in `localStorage` under `golish_session`.
- Accepts:
  - `admin` / `admin`
