...
function buildCss() {
  const presetName = getStored("golish_theme_preset");
  const preset = presetName ? presets[presetName] : undefined;

  const lightPrimary = getStored("golish_theme_light_primary");
  const lightSecondary = getStored("golish_theme_light_secondary");
  const darkPrimary = getStored("golish_theme_dark_primary");
  const darkSecondary = getStored("golish_theme_dark_secondary");
...
export function ColorVarsProvider() {
  React.useEffect(() => {
    ...
    window.addEventListener("golish-theme-colors-updated", handler as EventListener);
    return () => {
      window.removeEventListener("golish-theme-colors-updated", handler as EventListener);
    };
  }, []);
  return null;
}
