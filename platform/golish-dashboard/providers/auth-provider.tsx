...
function readAuthFromCookie(): { token?: string; user?: User } {
  const raw = getCookieValue("golish_session");
  if (!raw) return {};
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const checkSession = () => {
      try {
        clearCookie("golish_cookie");
        clearCookie("golish_token");
        const fromStorageToken = localStorage.getItem("golish_token");
        const fromStorageUser = localStorage.getItem("golish_session");
        const forcedLoggedOut = localStorage.getItem("golish_force_logged_out") === "true";
        const fromCookie = forcedLoggedOut ? {} : readAuthFromCookie();

        const resolvedToken = fromStorageToken || fromCookie.token || null;
        if (resolvedToken) {
          if (!fromStorageToken) {
            localStorage.setItem("golish_token", resolvedToken);
          }
        ...
        if (fromCookie.user) {
          localStorage.setItem("golish_session", JSON.stringify(fromCookie.user));
        ...
        localStorage.removeItem("golish_session");
        localStorage.removeItem("golish_token");
        clearCookie("golish_session");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = React.useCallback(
    async (username: string, password: string) => {
      ...
      localStorage.removeItem("golish_force_logged_out");
      localStorage.setItem("golish_token", nextToken);
      writeCookie("golish_session", nextToken);
      setToken(nextToken);
      const userData: User = {
        id: `user-${Date.now()}`,
        username,
      ...
      localStorage.setItem("golish_session", JSON.stringify(userData));
      setUser(userData);
      router.replace("/");
    },
    [router]
