...
const DEMO_ARTIFACTS: Artifact[] = [
  {
    ...
    artifactPath: "/home/golish/workspaces-golish/bin.com/security-report.md",
    artifactType: "report",
    contentType: "md",
    sizeBytes: 0,
    lineCount: 0,
  ...
  },
  {
    ...
    sizeBytes: 8923,
    lineCount: 45,
    description: "失败前生成的部分 nuclei 结果",
...
export const WORKSPACE_PREFIX_KEY_STORAGE = "golish_workspace_prefix_key";
...
export async function fetchArtifactContent(params: {
  ...
  if (!workspace) {
    throw new Error("0:必须指定工作区");
  }
  if (!artifactPath) {
    throw new Error("0:必须指定产物路径");
  }

  if (isDemoMode()) {
    return `# 示例仓库发现 - ${workspace}\n\n**目标**：${workspace}\n**日期**：2026-01-10\n\n\n---\n*由安全测试平台生成*\n`;
  }
