...
const LIVE_API_HEADER = "X-Golish-Use-Live-API";
...
function resolveBaseURL(): string {
  ...
  if (typeof window !== "undefined") {
    const saved = localStorage.getItem("golish_api_endpoint");
    if (saved) {
...
http.interceptors.request.use((config) => {
  ...
  if (typeof window !== "undefined") {
    const token = localStorage.getItem("golish_token");
    if (token) {
...
http.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 401 && typeof window !== "undefined") {
      ...
      localStorage.setItem("golish_force_logged_out", "true");
      localStorage.removeItem("golish_token");
      localStorage.removeItem("golish_session");
      try {
        document.cookie = "golish_cookie=; Path=/; Max-Age=0; SameSite=Lax";
        document.cookie = "golish_token=; Path=/; Max-Age=0; SameSite=Lax";
        document.cookie = "golish_session=; Path=/; Max-Age=0; SameSite=Lax";
      } catch {}
