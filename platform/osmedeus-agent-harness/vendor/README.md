...
`cyberstrike-source.json` records the immutable source and expected index
...
the Osmedeus `pentagi_skill` tool.

To update the corpus, replace `cyberstrike-skills/` from the upstream
`.cyberstrike/skill` directory, update the source manifest, and run:

```bash
cd platform/osmedeus-agent-harness
