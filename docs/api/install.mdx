...
```bash
curl "http://localhost:8002/golish/api/registry-info?registry_url=https://example.com/my-registry.json" \
  -H "Authorization: Bearer $TOKEN"
```

**With a local file path:**
```bash
curl "http://localhost:8002/golish/api/registry-info?registry_url=/etc/golish/registry.json" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl "http://localhost:8002/golish/api/registry-info?registry_mode=nix-build&registry_url=/etc/golish/registry.json" \
  -H "Authorization: Bearer $TOKEN"
```
