...
queried across all of them at once. Golish derives a workspace per target —
...
```bash
curl http://localhost:8002/golish/api/orgs \
...
```bash
curl http://localhost:8002/golish/api/orgs/acme \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/orgs/acme/stats \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST http://localhost:8002/golish/api/orgs \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
curl -X PUT http://localhost:8002/golish/api/orgs/acme \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE http://localhost:8002/golish/api/orgs/acme \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X DELETE "http://localhost:8002/golish/api/orgs/acme?purge=true" \
  -H "Authorization: Bearer $TOKEN"
```
...
| Endpoint | Behavior |
|----------|----------|
| `GET /golish/api/assets` | Assets across every workspace in the org |
| `GET /golish/api/vulnerabilities` | Vulnerabilities across the org |
| `GET /golish/api/runs` | Runs across the org |
| `GET /golish/api/workspaces` | Workspaces belonging to the org |

```bash
curl "http://localhost:8002/golish/api/vulnerabilities?org=acme&severity=high" \
  -H "Authorization: Bearer $TOKEN"
```

## Attributing new scans to an org

Runs created through `POST /golish/api/runs` inherit the org of the target's
...
```bash
golish run -f general -t acme.com --org acme
```
