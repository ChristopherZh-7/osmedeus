...
```bash
curl http://localhost:8002/golish/api/artifacts \
  -H "Authorization: Bearer $TOKEN"
```

**With pagination:**
```bash
curl "http://localhost:8002/golish/api/artifacts?offset=0&limit=50" \
  -H "Authorization: Bearer $TOKEN"
```

**Filter by workspace:**
```bash
curl "http://localhost:8002/golish/api/artifacts?workspace=example.com" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl http://localhost:8002/golish/api/artifacts/example.com \
  -H "Authorization: Bearer $TOKEN" \
