...
```bash
curl http://localhost:8002/golish/api/event-receiver/status \
...
```bash
curl http://localhost:8002/golish/api/event-receiver/workflows \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST http://localhost:8002/golish/api/events/emit \
  -H "Authorization: Bearer $TOKEN" \
